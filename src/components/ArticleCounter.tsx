    ...
    <section aria-labelledby="stats-heading" className="mt-10 mb-10">
      ...
      <div className="bg-white rounded-xl ring-1 ring-slate-100 shadow-sm p-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((stat) => (
            <div key={stat.label} className="text-center p-4 rounded-lg bg-slate-50 ring-1 ring-slate-100" aria-label={stat.ariaLabel}>
              <div className="flex justify-center mb-2">
                <div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center ring-1 ring-amber-200/50">
                  {stat.icon}
                </div>
              </div>
              <p className="text-2xl font-bold text-slate-800 tracking-tight">{stat.value.toLocaleString()}</p>
              <p className="text-[11px] text-slate-500 font-medium mt-0.5 tracking-wide uppercase">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
