  ...
  return (
    <footer className="bg-[#0F172A] mt-16" role="contentinfo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-6 h-6 rounded bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center font-bold text-white text-[10px]" aria-hidden="true">
                S
              </div>
          ...
          <nav aria-label="Footer quick links">
            <h2 className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-3">Quick Links</h2>
            <ul className="space-y-1.5">
              {footerQuickLinks.map(link => (
                <li key={link.page + link.label}>
                  <button
                    onClick={() => onNavigate(link.page, link.category)}
                    className="text-[12px] text-slate-400 hover:text-white transition-colors duration-200"
                  >
                    {link.label}
                  </button>
                </li>
          ...
          <nav aria-label="Footer categories">
            <h2 className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-3">Categories</h2>
            <ul className="space-y-1.5">
        ...
        {/* Bottom Bar */}
        <div className="border-t border-white/[0.06] mt-8 pt-4 flex flex-col sm:flex-row items-center justify-between gap-2">
